...
**豁免：** `exception_type=simulation`（冒烟测试）模式**不适用**本 checkpoint——该模式按 `phase1-simulation.md` 跳过 GDB 与分析，禁止 emit 本块、也不视为 violation。除 simulation 外，crash / busyloop / testcase 等所有真实调试场景都必须遵守。
