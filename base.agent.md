...
1. **必须按 `exception_type` 选对分析 skill 链**（见下方"异常分诊"）：`crash`/`busyloop`/`testcase` 用 `crash-analysis`+`gdb-start`；`ui`/`graphics` 用 `ui-bugfix`/`quickapp-bug-fix`+`experience-graphic`（无 GDB/coredump）
...
进入 Phase 1 根因分析时，**必须**先读取与 `exception_type` 匹配的 skill 的 SKILL.md 文件，按其工作流执行分析，禁止跳过或用自身知识替代：

| Skill | 用途 | 读取路径 | 适用 exception_type |
|-------|------|---------|---------|
| `crash-analysis` | GDB crash 根因分析 | `/root/.claude/skills/crash-analysis/SKILL.md` | crash / busyloop / testcase |
| `gdb-start` | GDB 会话启动与管理 | `/root/.claude/skills/gdb-start/SKILL.md` | crash / busyloop / testcase |
| `ui-bugfix` | miwear UI bug 端到端解决 | `/root/.claude/skills/ui-bugfix/SKILL.md` | ui / graphics（miwear 栈） |
| `quickapp-bug-fix` | QuickApp/.ux UI bug 端到端解决 | `/root/.claude/skills/quickapp-bug-fix/SKILL.md` | ui / graphics（quickapp 栈） |
| `experience-graphic` | 图形经验卡路由(Gate-1)与回流(Gate-2) | `/root/.claude/experience-graphic/`（router.md / compile_kb.py / append_outcome.py） | ui / graphics |

---

## 异常分诊（exception_type → Phase 1 skill 链 + 输入形态）

Agent 启动后**第一步按 `exception_type` 分诊**，决定 Phase 1-3 走哪条链；Phase 4-5（提交/导出/结果 JSON）所有类型共用。

| exception_type | Phase 1 片段 | Phase 1-3 skill 链 | 主要输入 | 用 GDB? |
|---|---|---|---|---|
| `crash` | phase1-crash.md | crash-analysis + gdb-start | elf_path + coredump/gdb_port | ✅ |
| `busyloop` | phase1-busyloop.md | crash-analysis + gdb-start | elf_path + gdb_port | ✅ |
| `testcase` | phase1-testcase-failure.md | crash-analysis + gdb-start + pytest 双栈 | log_path + test_case + elf_path | ✅/fallback |
| `ui` / `graphics` | **phase1-ui.md** | **栈分诊 → ui-bugfix 或 quickapp-bug-fix + experience-graphic Gate-1/2** | **jira_id / 截图 / .ux 源码 / component**（无 coredump/elf） | ❌ |

**UI/graphics 分支与 crash 分支的关键差异**（Phase 1-3 行为不同，务必区分）：
- **无 GDB/coredump**：跳过本文件所有 GDB MCP / coredump / nxstub 步骤；源码定位靠 JIRA 描述 + 截图 + 栈分诊，不靠 backtrace。
- **先栈分诊**：UI bug 必须先判 miwear 栈还是 quickapp 栈（见 ui-bugfix `routing.md §一 栈分诊表`），再进对应 sibling skill；判错 bounce。
- **经验回流闭环**：Phase 1 前跑 experience-graphic **Gate-1**（compile_kb.py --auto + 栈过滤路由,注入命中卡）；Phase 4 commit 后跑 **Gate-2**（append_outcome 记 routing feedback + graphics-bug-card 模式 B 起草候选卡到 candidates/，不写 units/）。
- **`elf_path` 对 UI 类非必填**（见输入上下文表"必填"列的条件说明）。
...
| 字段 | 类型 | 必填 | 说明 | 使用阶段 |
|------|------|------|------|---------|
| `exception_type` | string | 是 | 异常类型：`crash`/`busyloop`/`testcase`/`ui`(=`graphics`) | 类型识别 + 分诊 |
| `elf_path` | string | 条件 | ELF 文件路径（含调试符号）。**crash/busyloop/testcase 必填；ui/graphics 不需要** | P1-P2: GDB 加载 |
...
| `gdb_port` | int/str | 否 | GDB server 端口/socket，有则直连（仅 crash 系） | P1-P2 |
| `gdb_script` | string | 否 | GDB 初始化脚本路径（仅 crash 系） | P1-P2 |
| `core_dump_path` | string | 否 | coredump 文件路径（仅 crash 系） | P1-P2 |
| `log_path` | string | 否 | 运行日志路径 | P1-P2 |
| `test_case` | string | 否 | 触发异常的测试用例（pytest 格式） | P1-P2 |
| `jira_id` | string | 否 | JIRA issue ID（**ui/graphics 常用**：VELAPLATFO-* / MIWEARRTOS-*） | P1: 取现象/截图/版本 |
| `screenshot_path` | string | 否 | 截图/录屏路径（**ui/graphics**：现象证据） | P1 |
| `ux_source` | string | 否 | quickapp `.ux` 源码路径（**ui/graphics·quickapp 栈**） | P1-P2 |
| `component` | string | 否 | JIRA component / 应用包名（**ui/graphics**：用于栈分诊 miwear vs quickapp） | P1: 栈分诊 |
...
`ui` / `graphics` 类示例（**无 elf/coredump/gdb，靠 JIRA + 截图 + 栈分诊**）：
...
```json
{
    ...
    "product": "p62",
    "branch": "trunk-26rc01",
    "jira_id": "VELAPLATFO-90062",
    "component": "Vela 快应用",
    "screenshot_path": "/xxx/issue_90062_screen.png",
    "ux_source": "/xxx/app/pages/index/index.ux",
    "source_root": "/xxxx/vela",
    "workspace": "/xxxx/rwt/ws-90062",
    "result_output_dir": "/xxxx/out"
}
```

> ⚠️ ui/graphics 类**不要**传 `elf_path` / `core_dump_path` / `gdb_port`（即使传了也忽略）；定位靠 `jira_id`/`screenshot_path`/`component`（栈分诊）+ Gate-1 命中卡。

---

### GDB 调试方式（必须通过 MCP 工具）（仅 `crash`/`busyloop`/`testcase`）

> `ui`/`graphics` 类**不进入本节**（无 GDB/coredump）——分诊为 ui 时直接跳过 GDB 全部步骤，按 phase1-ui.md 走。
...
> **ui/graphics 类**：跳过下面 GDB/backtrace 项，改用 ui-bugfix/quickapp-bug-fix 的 `code-map` + 栈分诊结果 + Gate-1 命中卡的"排查路径"定位（详见 phase1-ui.md）。下列 1-4 仅适用 crash 系。
...
- **（仅 `testcase`）修改 VTF 引擎代码**（测试框架共享基础设施）：
  - `tests/tools/vtf/lib/**` / `tests/tools/vtf/engine/**` / `tests/tools/vtf/container/**`
  - `tests/tools/vtf/configs/**` / `tests/tools/vtf/docs/**`
  - `tests/tools/engine/**`
  - testcase 类 **只改被测程序和 pytest 用例脚本**；如根因定位到 VTF 引擎，进 Phase 5 报 `vtf_engine_bug`，不要自己动手修框架

**修改范围按 `exception_type` 分支**（除"严禁改 source_root"对所有类型生效外）：

| exception_type | 可改 | 禁改 / 越界即报 |
|---|---|---|
| `crash` / `busyloop` | 定位到的 NuttX/frameworks/apps 源码 | — |
| `testcase` | 被测 C 程序 + pytest 用例脚本 | VTF 引擎（上表）→ `vtf_engine_bug` |
| `ui` / `graphics` | `frameworks/runtimes/quickapp`、`frameworks/runtimes/feature`、`frameworks/graphics/uikit`、`vendor/xiaomi/miwear/apps` 等框架/widget/样式/资源代码 | 三方 `.ux` 业务逻辑（不在本仓）→ `out_of_scope`；BSP/驱动/panel → `wrong_exception_type` |
...
**🛑 硬约束：被测程序源码在 base 里缺失 → 立刻 abort，禁止重建（N1，仅 `testcase`/`crash`/`busyloop`）**

> **`ui`/`graphics` 类不适用本检查**——UI bug 没有"被测程序"概念，源码缺失/版本错位由 phase1-ui.md 的 **Step 1.5 版本对齐**判定：三条对齐路径全失败才报 `source_missing_on_base`，**不要**用 `apps/testing` 模型对 UI 误 abort。

修复前必查（crash 系）：被测试用例调用的程序源码（如 `apps/testing/<name>/<name>_main.c`）是否存在于 `<base_ref>` HEAD：
...
### 4.4 （仅 `ui`/`graphics`）Gate-2 经验回流（commit 后强制，零摩擦）

> 闭环后半段 `Fix → Feedback → Experience`。crash 系由 `experience-generator` 处理，**本节仅 ui/graphics 类执行**。

1. **记 outcome（必做）**：`python3 /root/.claude/experience-graphic/append_outcome.py` 写全局 `_outcomes.jsonl`，填 `experience_interaction`：Gate-1 注入了哪些卡(cards_injected)、真用上方向对的(cards_adopted)、用了却被推翻的=误导(cards_contradicted)、事后该命中没命中的(cards_missed)。= routing feedback。
2. **沉淀判定 + 起草**：调 `graphics-bug-card` 模式 B / Post-Fix Harvest（H1 决策表，默认 no_card）。要沉淀则按机制查重（不按现象）→ 起草到 `experience-graphic/candidates/`（留 TODO），**不写 units/**（A/B 闸门 + 人审后才升级）。
3. 仅当 `root_cause_found 且有 fix diff` 才进沉淀分支；否则止于第 1 步记 outcome。

> ⚠️ **soft-fail（不阻塞交付）**：Gate-2 是经验增强层,**不得影响修复结果的提交/导出**。若运行环境**缺 `/root/.claude/experience-graphic/` 目录或脚本**(append_outcome.py / compile_kb.py / graphics-bug-card)→ **跳过本节、在 analysis_report 记一行 warning 即可,照常进 Phase 5**,绝不因经验回流失败把已成功的 patch 判为 failed。同理 Phase 1 的 Gate-1 缺目录/空库也 soft-fail。
...
- `diagnosis`：始终填充
- **失败/越界归属写进 `diagnosis.root_cause` 的 `<token>: <detail>` 前缀**（沿用既有约定，**不新增字段**，平台按 root_cause 文本消费）。`status` 同时置 `failed_to_fix`：
  - `source_missing_on_base: <path> not in <base_ref>`（crash/testcase：被测源码缺失；ui：Step 1.5 三路径全失败）
  - `vtf_engine_bug: <loc> <现象>`（testcase：根因在 VTF 引擎）
  - `out_of_scope: third_party_ux`（ui/graphics：根因在三方 `.ux` 业务逻辑，不在本仓）
  - `wrong_exception_type: <实际类型>`（ui/graphics：根因落到 BSP/驱动/panel/crash，类型填错，应改走对应分支）
  - 正常修复成功则 root_cause 写一句话根因，`status=patch_generated`
