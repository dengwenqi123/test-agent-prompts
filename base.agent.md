...
| `pid` | int | 否 | 异常任务 PID（NuttX PID，**非** GDB thread ID）。GDB 连上后用它在 `ps` 输出里定位崩溃任务（见下文「pid 字段的 GDB 用法」） | P1-P2 |
...
- gdb_mode: A(coredump) | B(live) | skipped
- gdb_session_id: <mcp 返回的 sid；skipped 时空>
- gdb_connect_done: yes | no
- target_done: yes | no          # Mode A: file <elf> + target nxstub <core>；Mode B: target remote <port>
...
- `no_core_dump_and_no_gdb_port` —— 两者皆无，GDB 无对象可加载
...
> 三类 skip 都**必须先真实尝试 `gdb_connect` + `target`** 并在 checkpoint 附报错输出；不尝试直接跳过一律视为 violation。`target_done` 取证：Mode A / Mode B 经 `mcp__gdb-mcp__gdb_command` 执行时取命令输出；Mode B 若 target 在 tmux 启动命令里完成（`-ex 'target remote'`），用 `tmux capture-pane` 输出含 `connected`/`Remote debugging` 字样取证。
